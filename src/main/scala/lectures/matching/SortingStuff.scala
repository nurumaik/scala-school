...
object SortingStuff extends App {
  ...
  //  // Метод должен положить вещь в правильную коробку
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
      case watches@Watches(_, price) if price > 1000 => stuffBox.copy(watches = watches :: stuffBox.watches)
      case boots@Boots("Converse", _) => stuffBox.copy(boots = boots :: stuffBox.boots)
